...
db.users.createIndex({ username: 1 }); //enables quick username_based search
...
db.songs.createIndex({ release_date: -1 }); //arranges songs from most recent to oldest release_date, supporting features such as "new releases" or "recently played"
...
db.playlist.createIndex({created_at: -1}) //orders playlist in chronological order of creation. Used for "Newest playlists" feature 
db.playlist.createIndex({ user_id: 1, created_at: -1 }); //combines user identification and creation time to retrieve a user's most recently created playlist efficiently
...
//FOLLOWS
db.follows.createIndex({followers: 1, followed_at: -1}) //efficient retrieval of a user's following list
db.follows.createIndex({following_users: 1, followed_at: -1}) //efficient retrieval of followed users by a user
